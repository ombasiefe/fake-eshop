...
export default function ProductError({ error }: Props) {
    ...
    let message = "We couldn't load the products. Please try again later.";
    let status: number | null = null;

    if (isRouteErrorResponse(error)) {
        status = error.status;

        // Safely extract message string from error.data
        const dataMessage =
        ...
        if (error.status === 404) {
            ...
            message = dataMessage || "The requested products could not be located.";
        } else if (error.status === 500) {
            title = "Server error";
            message = dataMessage || "There was a problem communicating with the server.";
        } else {
